...
class GMSCrypto:
    ...
    def _init_keys(self):
        ...
        try:
            ...
            if self.sm2_private_key.exists() and self.sm2_public_key.exists():
                if not self._load_sm2_keys():
                    print("SM2 密钥加载失败，重新生成")
    ...
    def _generate_sm2_keys(self) -> bool:
        ...
        使用 gmssl 库生成标准的 SM2 密钥对
        
        Returns:
            成功返回True，失败返回False
        """
        if not GMSSL_AVAILABLE:
            print("gmssl 库不可用，使用模拟密钥")
            self._create_mock_keys()
            return True
    ...
    def calculate_public_key_fingerprint(self, public_key: str) -> Optional[str]:
        """计算公钥指纹
    ...
    def sm2_encrypt(self, plaintext: str) -> Optional[str]:
        ...
        try:
            ...
            # 加密数据
            encrypted = self.sm2.encrypt(plaintext.encode('utf-8'))
        ...
        try:
            ...
            cert_info = {
                ...
                "validity": {
                    ...
                    "not_after": (datetime.now() + timedelta(days=3650)).strftime("%Y%m%d%H%M%SZ")
                },
            ...
            # 生成证书签名
            cert_data = json.dumps(cert_info, sort_keys=True, indent=2)
...
def test_gms_crypto():
    """测试国密加密功能"""
    print("=" * 60)
    print("测试国密加密功能 (SM2/SM3/SM4)")
    print("=" * 60)
    
    crypto = get_crypto()
    
    # 测试数据
    test_data = "Hello, 国密算法! 你好，世界！"
